...
sample_size = 50
n = rng.integers(10, 100, size=sample_size)
...
y_buffer = 1
freq.set(ylim=[-y_buffer, max(n) + y_buffer])
ax[1].set(ylim=[-(y_buffer / max(n)), 1 + (y_buffer / max(n))])
freq.grid(None)
...
with pm.Model(coords=coords) as binomial_regression_model:
    x = pm.ConstantData("x", data["x"], dims="i")
    n = pm.ConstantData("n", data["n"], dims="i")
    # priors
    beta0 = pm.Normal("beta0", mu=0, sigma=1)
    beta1 = pm.Normal("beta1", mu=0, sigma=1)
    # linear model
    mu = beta0 + beta1 * x
    p = pm.Deterministic("p", pm.math.invlogit(mu), dims="i")
    # likelihood
    pm.Binomial("y", n=n, p=p, observed=data["y"], dims="i")
