...
petal_length_pred <- seq(1, 7, length = 13)
...
library(bayesplot)
# ヒストグラム描画
mcmc_hist(mcmc_sample_raw, pars = c("Intercept", "beta", "sigma", "sigma"))
# カーネル密度推定
mcmc_dens(mcmc_sample_raw, pars = c("Intercept", "beta", "sigma", "sigma"))
# 信用区間
mcmc_intervals(
...
ggplot() +
  ...
  labs(y='petal_width', title ='Credible interval')
```
3-4: 予測区間を散布図上に表示
```{r}
# 50%予測区間と95%予測区間を取得
...
df_outlier <- data.frame(
  df_outlier,
  outlier = df_outlier$Petal.Width < df_outlier$lower | df_outlier$Petal.Width > df_outlier$upper
)
...
ggplot() +  
  ...
  geom_ribbon(data=df_qua_petal_width,
              ...
              alpha=2/6) +
  geom_line(data=df_qua_petal_width, aes(x=petal_length, y=`50%`), size=1) +
  geom_point(data=df_outlier, aes(x=Petal.Length, y=Petal.Width, color=outlier), size=2) +
