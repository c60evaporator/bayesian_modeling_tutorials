...
# MCMC実行
mcmc_result <- stan(
  file = "./stan/usecase6_multiple_regression_interactive.stan", # stanファイルのパス
...
traceplot(mcmc_result, pars = c("Intercept", "b_sepal_length", "b_species[1]", "b_species[2]", "b_inter[1]", "b_inter[2]", "sigma", "lp__")) +
  labs(title="Without warmup")
# トレースプロット（バーンイン期間を含む）
traceplot(mcmc_result, inc_warmup=T, pars = c("Intercept", "b_sepal_length", "b_species[1]", "b_species[2]", "b_inter[1]", "b_inter[2]", "sigma", "lp__")) +
...
library(bayesplot)
# ヒストグラム描画
mcmc_hist(mcmc_sample_raw, pars = c("Intercept", "b_sepal_length", "b_species[1]", "b_species[2]", "b_inter[1]", "b_inter[2]", "sigma"))
# カーネル密度推定
mcmc_dens(mcmc_sample_raw, pars = c("Intercept", "b_sepal_length", "b_species[1]", "b_species[2]", "b_inter[1]", "b_inter[2]", "sigma"))
# 信用区間
mcmc_intervals(
  mcmc_sample_raw, pars = c("Intercept", "b_sepal_length", "b_species[1]", "b_species[2]", "b_inter[1]", "b_inter[2]", "sigma"),
...
mcmc_b_inter <- mcmc_sample$b_inter
...
for (i in 1:N){
  ...
  # + sepal_length*交互作用係数のMCMCサンプル行列 %*% speciesのダミー変数ベクトル
  mcmc_mu[,i] <- mcmc_Intercept + mcmc_b_sepal_length*df_iris$sepal_length[i] + (mcmc_b_species %*% species_dummy[i,])[,1] + df_iris$sepal_length[i]*(mcmc_b_inter %*% species_dummy[i,])[,1]
